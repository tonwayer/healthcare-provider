...
export default function ProviderDetail() {
  ...
  return (
    <main className="p-6">
      ...
      <p><strong>Status:</strong> <span aria-live="polite">{provider.deactivated ? "Inactive" : "Active"}</span></p>
      <Link href={`/providers/${provider.id}/edit`} className="bg-yellow-500 text-white p-3 rounded mr-2">
        Edit Provider
      </Link>
      {provider.deactivated ? (
        <button onClick={handleReactivate} className="bg-green-500 text-white p-2 rounded mt-4" aria-label={`Reactivate ${provider.name}`}>
          Reactivate Provider
        </button>
      ) : (
        <button onClick={handleDeactivate} className="bg-red-500 text-white p-2 rounded mt-4" aria-label={`Deactivate ${provider.name}`}>
          Deactivate Provider
        </button>
      )}
    </main>
